...
void FLASH_UnlockAllPages(void)
{
    ...
    CW_FLASH->PAGELOCK2 = 0x5A5Affff;
    CW_FLASH->PAGELOCK3 = 0x5A5Affff;
    CW_FLASH->PAGELOCK4 = 0x5A5Affff;
}
...
void FLASH_LockAllPages(void)
{
    ...
    CW_FLASH->PAGELOCK2 = 0x5A5A0000;
    CW_FLASH->PAGELOCK3 = 0x5A5A0000;
    CW_FLASH->PAGELOCK4 = 0x5A5A0000;
}

/**
 * @brief 解锁指定页面
 *
 * @param Page_Number : 0 - 511
...
uint8_t FLASH_UnlockPage(uint16_t Page_Number)
{
    ...
    switch (Page_Number / 128)
    {
        case 0:
            ...
            break;
        case 1:
            Currentlockstatus = CW_FLASH->PAGELOCK2;   //读取当前锁定位
            CW_FLASH->PAGELOCK2 = 0x5A5A0000 | Currentlockstatus | (1UL << ((Page_Number - 128) / 8));   //解锁指定页面
            break;
        case 2:
            Currentlockstatus = CW_FLASH->PAGELOCK3;   //读取当前锁定位
            CW_FLASH->PAGELOCK3 = 0x5A5A0000 | Currentlockstatus | (1UL << ((Page_Number - 256) / 8));   //解锁指定页面
            break;
        case 3:
            Currentlockstatus = CW_FLASH->PAGELOCK4;   //读取当前锁定位
            CW_FLASH->PAGELOCK4 = 0x5A5A0000 | Currentlockstatus | (1UL << ((Page_Number - 384) / 8));   //解锁指定页面
            break;
    }
    return(0x00);
}
 ...
 *
 * @param StartAddr : 0x00000000 - 0x0003FFFF
 * @param EndAddr   : 0x00000000 - 0x0003FFFF
...
uint8_t FLASH_UnlockPages(uint32_t StartAddr, uint32_t EndAddr)
{
    ...
    if ((StartAddr > 0x0003FFFF) || (EndAddr > 0x0003FFFF))
    {
    ...
    Currentlockstatus = CW_FLASH->PAGELOCK2;   //读取当前锁定位
    CW_FLASH->PAGELOCK2 = 0x5A5A0000 | Currentlockstatus | *(ptr + 1);   //解锁页面所在的区块
    Currentlockstatus = CW_FLASH->PAGELOCK3;   //读取当前锁定位
    CW_FLASH->PAGELOCK3 = 0x5A5A0000 | Currentlockstatus | *(ptr + 2);   //解锁页面所在的区块
    Currentlockstatus = CW_FLASH->PAGELOCK4;   //读取当前锁定位
    CW_FLASH->PAGELOCK4 = 0x5A5A0000 | Currentlockstatus | *(ptr + 3);   //解锁页面所在的区块
...
uint8_t FLASH_WriteBytes(uint32_t WriteAddr, uint8_t* pWrBuf, uint16_t WrByteCnt)
{
    ...
    if (WriteAddr + WrByteCnt > 0x00040000)
    {
        return(FLASH_ERROR_ADDR);
    }
...
uint8_t FLASH_WriteHalfWords(uint32_t WriteAddr, uint16_t* pWrBuf, uint16_t WrByteCnt)
{
    ...
    if (WriteAddr + (WrByteCnt<<1) > 0x00040000)
    {
        return(FLASH_ERROR_ADDR);
    }
...
uint8_t FLASH_WriteWords(uint32_t WriteAddr, uint32_t* pWrBuf, uint16_t WrByteCnt)
{
    ...
    if (WriteAddr + (WrByteCnt<<2) > 0x00040000)
    {
        return(FLASH_ERROR_ADDR);
    }
...
uint8_t FLASH_LockPage(uint16_t Page_Number)
{
    ...
    switch (Page_Number / 128)
    {
        ...
        case 1:
            Currentlockstatus = CW_FLASH->PAGELOCK2;   //读取当前锁定位
            CW_FLASH->PAGELOCK2 = (0x5A5A0000 | Currentlockstatus) & (~(1UL << ((Page_Number - 128) / 8)));  //锁定指定页面
            break;
        case 2:
            Currentlockstatus = CW_FLASH->PAGELOCK3;   //读取当前锁定位
            CW_FLASH->PAGELOCK3 = (0x5A5A0000 | Currentlockstatus) & (~(1UL << ((Page_Number - 256) / 8)));  //锁定指定页面
            break;
        case 3:
            Currentlockstatus = CW_FLASH->PAGELOCK4;   //读取当前锁定位
            CW_FLASH->PAGELOCK4 = (0x5A5A0000 | Currentlockstatus) & (~(1UL << ((Page_Number - 384) / 8)));   //锁定指定页面
            break;
    }
...
uint8_t FLASH_LockPages(uint32_t StartAddr, uint32_t EndAddr)
{
    ...
    Currentlockstatus = CW_FLASH->PAGELOCK2;   //读取当前锁定位
    CW_FLASH->PAGELOCK2 = 0x5A5A0000 | (Currentlockstatus & (~*(ptr + 1)));   //锁定页面所在的区块
    Currentlockstatus = CW_FLASH->PAGELOCK3;   //读取当前锁定位
    CW_FLASH->PAGELOCK3 = 0x5A5A0000 | (Currentlockstatus & (~*(ptr + 2)));   //锁定页面所在的区块
    Currentlockstatus = CW_FLASH->PAGELOCK4;   //读取当前锁定位
    CW_FLASH->PAGELOCK4 = 0x5A5A0000 | (Currentlockstatus & (~*(ptr + 3)));   //锁定页面所在的区块
